...
public class SqlValidatorTests
{
  ...
  public void RewriteTickIntervals_BracketExpansionWithoutDollarOrDuration_Rewrites()
  {
    ...
    Assert.Contains("ts BETWEEN TIMESTAMP '2026-03-05 00:00:00.000000' AND TIMESTAMP '2026-03-06 23:59:59.999999'", result);
  }

  [Fact]
  public void RewriteTickIntervals_TimezoneOffset_RewritesToUtcWindow()
  {
    var sql = "SELECT * FROM logs WHERE ts IN '2026-03-05@+02:00'";
    var result = SqlValidator.RewriteTickIntervals(sql, FixedNow);

    Assert.Contains("ts BETWEEN TIMESTAMP '2026-03-04 22:00:00.000000' AND TIMESTAMP '2026-03-05 21:59:59.999999'", result);
  }

  [Fact]
  public void RewriteTickIntervals_IanaTimezone_RewritesToUtcWindow()
  {
    var sql = "SELECT * FROM logs WHERE ts IN '2026-03-05@Europe/Rome'";
    var result = SqlValidator.RewriteTickIntervals(sql, FixedNow);

    Assert.Contains("ts BETWEEN TIMESTAMP '2026-03-04 23:00:00.000000' AND TIMESTAMP '2026-03-05 22:59:59.999999'", result);
  }

  [Fact]
  public void RewriteTickIntervals_VariableWithTimeSuffix_RewritesToPointWindow()
  {
    var sql = "SELECT * FROM logs WHERE ts IN '$todayT19:30'";
    var result = SqlValidator.RewriteTickIntervals(sql, FixedNow);

    Assert.Contains("ts BETWEEN TIMESTAMP '2025-06-15 19:30:00.000000' AND TIMESTAMP '2025-06-15 19:30:00.000000'", result);
  }

  [Fact]
  public void RewriteTickIntervals_TimeListWithDuration_RewritesToMergedWindow()
  {
    var sql = "SELECT * FROM logs WHERE ts IN '2026-03-05T[09:00,10:30];2h'";
    var result = SqlValidator.RewriteTickIntervals(sql, FixedNow);

    Assert.Contains("ts BETWEEN", result);
    Assert.Contains("2026-03-05 09:00:00.000000", result);
    Assert.Contains("2026-03-05 12:30:00.000000", result);
  }
